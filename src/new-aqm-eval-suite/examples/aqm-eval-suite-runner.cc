...
void RunOneScenario (std::string scenarioName)
{
  ...
  if (AggressiveTcp != "" && scenarioName == "aggressive-transport-sender")
    {
      commandToRun = std::string ("./ns3 run \"") + scenarioName + std::string (" --TcpVariant=ns3::") + AggressiveTcp + std::string (" --QueueDiscMode=") + QueueDiscMode + std::string (" --isBql=") + isBql + std::string ("\"");
    }
  else
    {
      commandToRun = std::string ("./ns3 run \"") + scenarioName + std::string (" --QueueDiscMode=") + QueueDiscMode + std::string (" --isBql=") + isBql + std::string ("\"");
    }
  uint32_t systemRet = system (commandToRun.c_str ());
  if(systemRet == -1){
  ...
  for (uint32_t i = 0; i < nAQM; i++)
    {
      std::string proQdelThr = std::string ("python src/new-aqm-eval-suite/utils/generate-ellipseinput.py ") + scenarioName + " " + AQM[i] + queueDisc;
      std::string proEllipse = std::string ("python src/new-aqm-eval-suite/utils/ellipsemaker ") + scenarioName + " " + AQM[i] + queueDisc + " " + std::to_string (i + 1);
      std::string plotGoodput = std::string ("python src/new-aqm-eval-suite/utils/goodput_process.py ") + scenarioName + " " + AQM[i] + queueDisc;
      std::string plotDelay = std::string ("python src/new-aqm-eval-suite/utils/delay_process.py ") + scenarioName + " " + AQM[i] + queueDisc;
...
void RunRttFairness (std::string scenarioName)
{
  ...
  std::string commandToRun = std::string ("./ns3 run \"rtt-fairness") + std::string (" --QueueDiscMode=") + QueueDiscMode + std::string (" --isBql=") + isBql + std::string ("\"");
      ...
      for (uint32_t i = 0; i < nAQM; i++)
        {
          ...
          std::string plotGoodput = std::string ("python src/new-aqm-eval-suite/utils/goodput_process.py ") + scenarioName + " " + AQM[i] + queueDisc;
          std::string plotDelay = std::string ("python src/new-aqm-eval-suite/utils/delay_process.py ") + scenarioName + " " + AQM[i] + queueDisc;
          std::string plotDrop = std::string ("python src/new-aqm-eval-suite/utils/drop_process.py ") + scenarioName + " " + AQM[i] + queueDisc;
          system (proQdelThr.c_str ());
          system (proEllipse.c_str ());
          system (plotGoodput.c_str ());
          system (plotDelay.c_str ());
...
int main (int argc, char *argv[])
{
  ...
  ScenarioNumberMapping["5.1.1"] = "tcp-friendly-same-initcwnd";
  ScenarioNumberMapping["5.1.2"] = "tcp-friendly-different-initcwnd";
  ScenarioNumberMapping["5.2"] = "aggressive-transport-sender";
  ScenarioNumberMapping["5.3.1"] = "single-unresponsive-transport";
  ScenarioNumberMapping["5.3.2"] = "unresponsive-with-friendly-transport";
  ScenarioNumberMapping["5.4"] = "lbe-transport-sender";
  ScenarioNumberMapping["8.2.2"] = "mild-congestion";
  ScenarioNumberMapping["8.2.3"] = "medium-congestion";
  ScenarioNumberMapping["8.2.4"] = "heavy-congestion";
  ScenarioNumberMapping["8.2.5"] = "varying-congestion";
  ScenarioNumberMapping["8.2.6.1"] = "varying-bandwidth-uno";
  ScenarioNumberMapping["8.2.6.2"] = "varying-bandwidth-duo";
  ScenarioNumberMapping["6"] = "rtt-fairness";
  ...
  if (scenarioName != "All" && scenarioName != "rtt-fairness")
    {
      RunOneScenario (scenarioName);
    }
  else if (scenarioName != "All" && scenarioName == "rtt-fairness")
    {
